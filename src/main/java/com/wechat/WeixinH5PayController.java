...
public class WeixinH5PayController {
    ...
    // pay 微信支付商户号
    String mchId = WebPropertiesUtil.getInstance().getValue(weixinData.MCHID);

    //mp.weixin  微信公共账号 appId
    String appId = WebPropertiesUtil.getInstance().getValue(weixinData.APPID);

    // 申请开通微信支付后，发给开发者。用于计算签名
    String apiKey = WebPropertiesUtil.getInstance().getValue(weixinData.KEY);

    // 微信支付 统一下单接口
    String payOrderUrl = WebPropertiesUtil.getInstance().getValue(weixinData.UNIFIED_ORDER_URL);

    // 支付 回调url
    String notifyUrl = WebPropertiesUtil.getInstance().getValue(weixinData.NOTIFY_URL_PAY) ;
     ...
     * voucherId  客户订单号
     */
    String openId = "oR40ut4DijoTrus-197ZH4ktSPF8";
